...
from metaworld.benchmarks import ML1, ML10, MT10
...
def make_env(env_id, seed, rank, log_dir=None, obs_keys=None, allow_early_resets=False, save_video=False, max_steps=None):
    def _thunk():
...
def make_vec_envs(env_id,
                  ...
                  evaluating=False):
    envs = [
        make_env(env_id, seed, i, log_dir, obs_keys,
                 allow_early_resets, save_video, max_steps=max_steps)
...
class MetaworldWrapper(gym.Wrapper):
    """
    Concatenates objective with observation and set reward as sparse
    Reward: -1 if task is not completed, 0 otherwise
    Observations are composed of [joint positions, joint deltas (to last observation), objective]
    """
