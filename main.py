...
if __name__ == '__main__':
    ...
    logger.configure(log_dir, ['stdout', 'log', 'csv'],
                     tbX=args.use_tensorboard)
    ...
    if args.checkpoint is not None:
        ...
        # set same statistics for normalization as in last training
    ...
    for update in range(num_updates):
        ...
        for step in range(args.num_steps):
            ...
            # store experience
            agent.store_rollout(obs[1], action, action_log_probs, value, reward, intrinsic_reward, done)
        ...
        if update % args.log_interval == 0:
            ...
            if (update + 1) % args.checkpoint_interval == 0:
                agent.save_checkpoint(
                    os.path.join(log_dir, 'checkpoint_' + str(update) + '.pth'), 
                    hasattr(envs.venv, 'ob_rms') and envs.venv.ob_rms or None # save vec_normalize statistics
                )
